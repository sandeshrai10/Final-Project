document.addEventListener("DOMContentLoaded", () => {
  document
    ...
    .addEventListener("click", loadManageEquipment);
  document
    .getElementById("manage-orders-btn")
    .addEventListener("click", redirectToRentalOrders);
  document
    .getElementById("manage-feedback-btn")
  ...
  fetch("/api/admin/equipment")
    ...
    .then((data) => {
      ...
      data.forEach((equipment) => {
        ...
        row.innerHTML = `
                    <td>${equipment.id}</td>
                    <td id="itemNumber-${equipment.id}">${
          equipment.itemNumber
        }</td>
                    <td><input type="text" id="edit-name-${
                      equipment.id
                    }" value="${equipment.name}" disabled></td>
                    <td><textarea id="edit-description-${
                      equipment.id
                    }" disabled>${equipment.description}</textarea></td>
                    <td>
                        <select id="edit-category-${equipment.id}" disabled>
                            <option value="Cameras" ${
                              equipment.category === "Cameras" ? "selected" : ""
                            }>Cameras</option>
                            <option value="Headphones" ${
                              equipment.category === "Headphones"
                                ? "selected"
                                : ""
                            }>Headphones</option>
                            <option value="Laptops" ${
                              equipment.category === "Laptops" ? "selected" : ""
                            }>Laptops</option>
                            <option value="Tablets" ${
                              equipment.category === "Tablets" ? "selected" : ""
                            }>Tablets</option>
                        </select>
                    </td>
                    <td><input type="number" id="edit-stockQuantity-${
                      equipment.id
                    }" value="${equipment.stockQuantity}" disabled></td>
                    <td><input type="number" id="edit-dailyRentalRate-${
                      equipment.id
                    }" value="${equipment.dailyRentalRate}" disabled></td>
                    <td>
                        <select id="edit-status-${equipment.id}" disabled>
                            <option value="Available" ${
                              equipment.availability ? "selected" : ""
                            }>Available</option>
                    ...
                    <td><button class="edit" onclick="enableEditing(${
                      equipment.id
                    })">Edit</button></td>
                    <td><button class="delete" onclick="deleteEquipment(${
                      equipment.id
                    })">Delete</button></td>
                    <td><button class="save" id="save-${
                      equipment.id
...
function addEquipment(event) {
  ...
  fetch("/api/admin/equipment")
    ...
    .then((data) => {
      ...
      alert("Equipment added successfully!"); // Optional: show a success message
      loadEquipmentList(); // Refresh the equipment list
      document.getElementById("add-equipment-form").reset(); // Clear the form fields
    })
    .catch((error) => {
...
function saveEquipment(id) {
  const updatedEquipment = {
    id: id,
    itemNumber: document.getElementById(`itemNumber-${id}`).textContent, // Use textContent to get the non-editable item number
    ...
    dailyRentalRate: document.getElementById(`edit-dailyRentalRate-${id}`)
      .value,
    availability:
      document.getElementById(`edit-status-${id}`).value === "Available",
  };
