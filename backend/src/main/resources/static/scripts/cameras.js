...
const cameras = [
  ...
  },
  {
    ...
    autofocus:
      "Fast Hybrid AF with 693 phase-detection and 425 contrast-detection points",
  ...
  },
  {
    ...
    autofocus:
      "Fast Hybrid AF with 425 phase-detection and 425 contrast-detection points",
    video: "4K HDR at 30p, Full HD at 120p",
...
function displayCameras(filteredCameras) {
  if (!Array.isArray(filteredCameras)) {
    console.error(
      "Expected filteredCameras to be an array, got:",
      typeof filteredCameras
    );
    return;
  }
    ...
    cameraItem.innerHTML = `
            <div class="image-gallery">
                <div class="main-image">
                    <img src="${
                      camera.images[camera.currentImageIndex]
                    }" alt="${
      camera.name
    }" width="300" height="200" id="mainImage${camera.itemNumber}">
                </div>
                <div class="arrows">
                    <button class="prev" onclick="changeCameraImage(-1, ${
                      camera.itemNumber
                    })">&#10094;</button>
                    <button class="next" onclick="changeCameraImage(1, ${
                      camera.itemNumber
                    })">&#10095;</button>
                </div>
            </div>
            <div class="camera-details">
                ...
                <button class="btn" onclick="checkCameraAvailability(${
                  camera.itemNumber
                })">Check Availability</button>
                <div class="rating" id="rating-button-${
                  camera.itemNumber
                }" onclick="navigateToReviews(${camera.itemNumber})">
                    Loading...
                </div>
            </div>
        `;
...
window.changeCameraImage = function (direction, itemNumber) {
  ...
  document.getElementById(`mainImage${itemNumber}`).src =
    camera.images[camera.currentImageIndex];
};
