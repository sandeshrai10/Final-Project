    ...
    function fetchAllReviews() {
        fetch('/api/reviews')
            ...
            .then(reviews => {
                ...
                if (reviews.length === 0) {
                    reviewsContainer.innerHTML = '<p id="placeholder-text">No reviews available.</p>';
                } else {
    ...
    function searchReviews(itemNumber) {
        fetch(`/api/reviews/item/${itemNumber}`)
            ...
            .then(reviews => {
                reviewsContainer.innerHTML = '';
                if (reviews.length === 0) {
                    reviewsContainer.innerHTML = '<p id="placeholder-text">No reviews found for this item number.</p>';
                } else {
                    reviews.forEach(review => {
                        displayReview(review);
                    });
