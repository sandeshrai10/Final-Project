document.addEventListener("DOMContentLoaded", () => {
  ...
  reviewForm.addEventListener("submit", (event) => {
    ...
    const titleWordCount = countWords(reviewData.title);
    const commentsWordCount = countWords(reviewData.comments);

    if (titleWordCount > 500) {
      errorMessage.textContent = "Title cannot exceed 200 words.";
      errorMessage.style.color = "red";
      reviewForm.insertBefore(errorMessage, reviewForm.firstChild);
      return;
    }

    if (commentsWordCount > 10000) {
      errorMessage.textContent = "Comments cannot exceed 1000 words.";
      errorMessage.style.color = "red";
      reviewForm.insertBefore(errorMessage, reviewForm.firstChild);
      return;
    }
  ...
  function fetchReviews() {
    ...
    fetch(`/api/reviews/item/${itemNumber}`)
      ...
      .then((reviews) => {
        ...
        if (reviews.length === 0) {
          reviewsContainer.innerHTML =
            '<p id="placeholder-text">No reviews yet. Be the first to write a review!</p>';
        } else {
  ...
  function isUserLoggedIn() {
    // Implement logic to check if user data is present in localStorage
