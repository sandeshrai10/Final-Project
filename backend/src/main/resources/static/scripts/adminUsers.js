document.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchAllUsers() {
    fetch("/api/users")
      ...
      .then((users) => {
        ...
        if (users.length === 0) {
          usersContainer.innerHTML =
            '<tr><td colspan="10" id="placeholder-text">No users found.</td></tr>';
        } else {
          users.forEach((user) => {
            displayUser(user);
          });
      ...
      .catch((error) => {
        ...
        usersContainer.innerHTML =
          '<tr><td colspan="10" id="placeholder-text">Failed to load users. Please try again later.</td></tr>';
      });
  }
  ...
  addUserForm.addEventListener("submit", (event) => {
    ...
    if (!/^\d{10}$/.test(user.phoneNumber)) {
      responseMessage.textContent = "Phone number must be exactly 10 digits.";
    ...
    fetch("/api/users/create", {
      ...
      .catch((error) => {
        responseMessage.textContent = `Error adding user: ${error.message}`;
        responseMessage.style.color = "red";
      });
  });
